...
- [~] Per-broker namespace model has a local operator implementation (`crates/broker-operator`) but is not yet applied to dev01.
  - Evidence: `kubectl get namespaces -l sage.suresure.ai/posp-id` returns nothing.
- [~] Broker Runtime Operator is locally scaffolded and compiles; it is not deployed to AKS yet.
  - Evidence: No operator/controller deployment in `sageinfra-new-agents` or any namespace.
- [~] Workload Identity ServiceAccount annotations are implemented in the local operator; Azure federated credential creation and live pod token validation are pending approval/deployment.
...
- [~] Frontend auth bypass has been removed from local dev01 manifest/chat path; live deployment still needs validation/redeploy.
  - Evidence: local `k8s/sageinfra-frontend-dev01.yaml` sets `NEXT_PUBLIC_DISABLE_AUTH=false`; chat paths require bearer auth and no `demo-token` fallback was found by `git grep` on 2026-05-16. Live dev01 has not been touched.
- [~] Frontend dashboard and first-collage workspace modules now have a local authenticated API contract instead of only static arrays/placeholders; Microsoft Fabric/live system-of-record data sources are still pending.
  - Evidence: local Express `/api/dashboard/ops`, `/api/runtime/status`, `/api/workspace/modules`, `/api/workspace/modules/:moduleId`, and `/api/workspace/modules/:moduleId/actions` require bearer auth and return broker-runtime/Fabric-hook contract data for Operations Home, Claims Chat, FNOL Intake, Underwriting Workbench, Policy Pulse, CRM Agent, Claims Lifecycle, Consumer Policy, and SageSure AI companion. `node --check frontend/server.mjs`, `npm --prefix frontend run build`, `npm --prefix frontend run smoke:workspace-contract` (credentialless skip), and local curl of `/healthz` plus authenticated/unauthenticated `/api/workspace/modules` passed on 2026-05-16.
...
- [x] Add `hidden: true` flag to `productAreas` items in `TabsInterface.tsx` for all 10 Hidden-needed modules:
  `tasks`, `claims-queue`, `uw-queue`, `scamshield`, `policy-pulse`, `claims-defender`, `document-vault`, `renewals`, `buying-assistance`, `producer`.
  - Filtered out `hidden: true` items in the sidebar render.
  - Requirements: 9.2
- [x] Remove hardcoded badge numbers for items without live data: `tasks` badge "12", `claims-queue` badge "31", `uw-queue` badge "18".
  - Requirements: 9.2

### 0.9 Remove Misleading Status Widget
- [x] Remove or replace hardcoded "dev01 online / AgentCore connected locally via secure port-forward" widget from `TabsInterface.tsx:637–641`.
  - Replaced first with a neutral placeholder, then wired the sidebar runtime-status card to authenticated `/api/runtime/status` backed by broker-runtime router health/capability data; unauthenticated state shows auth-required/not-connected rather than a fake live status.
  - Requirements: 11.4
...
- [x] Decision: use Rust `kube-rs` crate (consistent with `crates/` workspace). Evidence: root `Cargo.toml` workspace plus `crates/broker-operator/Cargo.toml` using `kube`/`k8s-openapi`; `cargo check -p broker-operator` passed 2026-05-16.
...
- [x] Create CRD YAML at `crates/broker-operator/crds/brokerruntime.yaml`. Evidence: CRD includes POSP/tenant/runtime/identity/workspace/KB/tools/skills/policy spec and status schema.
...
- [x] `crates/broker-operator/src/main.rs`: tokio async runtime; Controller for BrokerRuntime; error_policy; reconcile function signature. Evidence: `cargo check -p broker-operator` passed 2026-05-16.
...
- [x] In reconcile: create or patch `broker-{posp_id}` namespace. Evidence: `ensure_namespace` applies a namespace derived by `broker_namespace(posp_id)`.
...
- [x] Add finalizer `platform.sagesure.ai/broker-cleanup` to BrokerRuntime CR on creation. Evidence: `ensure_finalizer`, `cleanup_broker`, and `remove_finalizer` implemented locally and `cargo check -p broker-operator` passed 2026-05-16.
...
- [x] Create `broker-runtime` ServiceAccount in `broker-{posp_id}` namespace. Evidence: operator applies ServiceAccount with `azure.workload.identity/use=true` label and managed identity/client-id annotations.
...
- [x] Decide: one UAMI per broker (isolation, Azure RBAC scoping) vs. shared UAMI with per-resource conditions. Decision: local implementation uses one broker-scoped UAMI reference via `spec.identity.managedIdentityName` / optional `clientId`.
...
- [x] Apply per-namespace ResourceQuota (max pods, CPU, memory) and LimitRange (default container requests/limits). Evidence: `ensure_resource_quota` and `ensure_limit_range` implemented and compiled.
...
- [x] Apply default-deny-all ingress and egress NetworkPolicy. Evidence: `ensure_network_policies` applies `default-deny-all`.
- [~] Apply allowlist egress: DNS (UDP/TCP port 53 to kube-dns), Azure service endpoints (HTTPS 443), platform gateway ingress only. Local placeholder policy exists; exact kube-dns/Azure/platform selectors remain pending before live apply.
...
- [x] Create `Dockerfile` for broker runtime image. Evidence: `crates/broker-runtime/Dockerfile` builds `broker-runtime` and stages `/config`; runtime exposes `/healthz` and `/readyz` locally.
...
- [x] Operator creates Deployment in `broker-{posp_id}` with: Evidence: local operator deployment template now sets Kata runtime class, broker ServiceAccount, broker/config env, explicit CPU/memory resources, config/workspace mounts, non-root read-only security context, and `/healthz`/`/readyz` probes; `cargo check -p broker-operator` passed 2026-05-16.
...
- [~] Apply a BrokerRuntime CR to dev01 with `pospId: "test-123"`, `lifecycleState: Active`, `runtimeClassName: kata-vm-isolation`. Local sample exists at `examples/broker-runtime/test-123.yaml`; live apply is pending approval.
...
- [x] Add status API or `kubectl` query that shows for each BrokerRuntime:
...
- [x] Implement HTTP endpoint in broker runtime: `POST /chat` accepts `{message, conversationId, brokerId}`, returns streaming or JSON response. Evidence: Axum `/chat` validates `brokerId`, enforces tool/skill allowlists, persists conversation history, and returns JSON.
...
- [x] Define workspace layout: `/workspace/{broker_id}/` containing conversation history, uploaded docs, broker-specific config. Evidence: runtime persists conversations under `/workspace/{broker_id}/conversations/` and config loads from `/config`.
...
- [~] Runtime reads `/config/kb.yaml` on startup; connects to broker's Azure Cognitive Search index. Local manifest load is implemented; live Azure Search client connection is pending.
  - KB refs per BrokerRuntime spec; no cross-broker index access.
  - Requirements: 3.3

### 4.4 Tool Manifest Enforcement
- [x] Runtime reads `/config/tools.yaml`; only tools listed are accessible to skills. Evidence: runtime loads tools allowlist and rejects unlisted `tool` values in `/chat`.
  - Attempting to invoke an unlisted tool returns explicit "tool not approved" error.
  - Requirements: 3.4

### 4.5 Skill Manifest Enforcement
- [x] Runtime reads `/config/skills.yaml`; only skills listed are loaded. Evidence: runtime loads skills allowlist and rejects unlisted `skill` values in `/chat`.
...
- [~] Frontend `/api/azure-chat` proxy → replaced by Broker Runtime Router that reads broker context from auth token and forwards to `broker-{posp_id}` service. Evidence: local `frontend/server.mjs` decodes bearer JWT claims, resolves/authorizes `brokerId`, exposes `/api/me/brokers`, forwards `/chat` payloads to `BROKER_RUNTIME_ROUTER_URL`, and returns 401/403 for missing/unauthorized broker access. Local frontend server plus broker runtime were brought up on ports 13000/18080 and `/api/dashboard/ops` successfully read runtime data. On 2026-05-16, `/api/workspace/modules` and `/api/runtime/status` were added as the first-collage module contract layer and local curl verified 401 unauthenticated plus authenticated schema response. Live service routing is not deployed/validated yet.
...
- [x] Schema: `{event: "signup"|"update"|"suspend"|"resume"|"offboard", pospId, tenantId, brokerName, config: {tools[], skills[], kbRefs[]}, idempotencyKey}`. Evidence: `LifecycleEvent`, `LifecycleEventKind`, and `LifecycleConfig` structs implemented in `crates/broker-runtime/src/main.rs`.
  - Requirements: 5.1

### 5.2 Build Lifecycle API
- [x] Rust Axum or Python FastAPI service in `sageinfra-new-agents` namespace. Evidence: Rust Axum broker runtime exposes lifecycle routes; deployment to namespace remains a separate pending live action.
...
- [x] Idempotency key in request; deduplicate on received key; return same response on replay. Evidence: runtime stores lifecycle records by `idempotencyKey` and returns existing records on replay.
  - Requirements: 5.3, 5.4

### 5.4 Lifecycle Audit Log
- [x] Emit structured event log entry for every lifecycle state change: `{timestamp, pospId, event, initiator, outcome}`. Evidence: lifecycle handler emits structured `tracing` audit event with timestamp, POSP, event, tenant/broker, idempotency key, and outcome.
...
- [x] Remove `NEXT_PUBLIC_DISABLE_AUTH=true` from all K8s manifests. Evidence: local `git grep` found no tracked `NEXT_PUBLIC_DISABLE_AUTH=true`; `k8s/sageinfra-frontend-dev01.yaml` sets it to `false`.
...
- [x] Backend endpoint: `GET /me/brokers` — reads identity claims from JWT; returns list of POSP IDs the user is authorized for. Evidence: runtime exposes `/me/brokers`, requires bearer auth, and returns authorized broker list for the runtime.
  - Requirements: 8.2, 8.4

### 6.4 Implement Broker Runtime Router
- [~] Middleware or sidecar that: validates bearer token; extracts broker claim; checks user is authorized for that broker; forwards to `http://broker-{posp_id}-gateway.broker-{posp_id}.svc.cluster.local/chat`. Evidence: local Express router implements bearer presence check, broker claim extraction/authorization, and broker-aware `/chat` forwarding through configurable `BROKER_RUNTIME_ROUTER_URL`; exact in-cluster service URL and JWT signature validation remain pending.
  - Returns 403 if user is not authorized for the requested broker.
  - Requirements: 8.2, 8.4

### 6.5 Update Frontend Modules to Pass Broker Context
- [x] All chat/agent calls include `brokerId` from resolved POSP identity. Evidence: local API client resolves broker from explicit option, `localStorage("sageinfra.activeBrokerId")`, or `/me/brokers`; direct chat callers no longer fall back to `test-123` and omit `brokerId` when UI state is absent so the backend resolves it from JWT claims. `node --check frontend/server.mjs` and `npm --prefix frontend run build` passed 2026-05-16.
...
### 6.7 Wire Dashboard Graphics to Broker/Fabric Metrics
- [~] Replace static dashboard-only graphics with a real data contract. Evidence: `frontend/server.mjs` exposes `/api/dashboard/ops`, pulling broker health/status from `BROKER_RUNTIME_ROUTER_URL` and preferring `MS_FABRIC_METRICS_URL`/`FABRIC_METRICS_URL` when configured; `frontend/components/SageSureDashboard.tsx` refreshes metrics, analytics panels, queue rows, and next-best actions from that endpoint. Local validation: `node --check frontend/server.mjs`, `npm --prefix frontend run build`, and curl against local frontend+runtime returned live runtime metrics. Pending: provision/configure the actual Microsoft Fabric workspace/lakehouse/warehouse or semantic model endpoint and deploy the env vars to dev01.
  - Requirements: 9.1, 9.3, 11.2
...
- [~] 7.2 Track last activity timestamp in BrokerRuntime status; update on every handled request. Runtime tracks `last_activity` and exposes it via `/platform/brokers`; CR status update from request activity is pending.
...
- [~] 8.1 Structured logs: every request log entry includes `pospId`, `brokerNamespace`, `conversationId`, `skill`, `tool`, `latencyMs`, `outcome`. JSON tracing is enabled; full required field set per request is pending.
- [~] 8.2 BrokerRuntime status API: `GET /platform/brokers` returns list of all BrokerRuntime CRs with their status fields. Local runtime endpoint returns current runtime status; cluster-wide CR listing remains pending.
