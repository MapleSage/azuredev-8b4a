...
const SpecialistChatInterface: React.FC<SpecialistChatInterfaceProps> = ({
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // Send to Azure FastAPI backend via the client
      const response: AgentCoreResponse = await client.current.sendQuery(
        inputText.trim(),
        bearerToken,
        specialistId
      );
      ...
      const assistantMessage: Message = {
        ...
        timestamp: new Date(),
        specialist: specialistId,
        sources: response.azure_metadata?.sources || [],
        confidence: response.azure_metadata?.confidence,
        tokens_used: response.azure_metadata?.tokens_used,
      };
