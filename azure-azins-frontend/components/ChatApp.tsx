...
class StrandsClient {
  ...
  async sendMessage(
    ...
    try {
      ...
      // Add JWT token for backend authentication
      if (authToken) {
      ...
      return {
        ...
        timestamp: new Date(),
        agent: data.handled_by || specialist,
      };
    } catch (error: any) {
  ...
  const handleSendMessage = useCallback(
    async (content: string, files?: File[]) => {
      ...
      try {
        ...
        // Connect to AKS ingress endpoint
        const response = await fetch('https://agents.maplesage.com/chat', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            message: content,
            conversation_history: context,
            session_id: sessionData.conversationId,
            specialist: targetSpecialist
          })
        });
        ...
        const strandsMessage = {
          ...
          content: data.response || data.text || 'No response',
          timestamp: new Date(),
          agent: data.handled_by || targetSpecialist
        };
