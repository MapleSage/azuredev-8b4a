...
**🛡️ Secure & Reliable:**
• Protected by Azure Content Safety for responsible AI
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage: Message = {
        id: (Date.now() + 1).toString(),
        role: 'assistant',
        content: `❌ Error: ${error.message}\n\n*Note: Policy Assistant requires connection to the deployed Azure Container Apps with Streamlit app and Azure Cognitive Search for full functionality.*`,
        timestamp: new Date()
      }
    ...
    <div className="h-full flex flex-col bg-white">
      ...
      <div className="flex-shrink-0 bg-gray-50 p-4 border-b">
        <div className="flex flex-wrap gap-3 text-xs">
          <span className="bg-blue-100 text-blue-800 px-2 py-1 rounded">🧠 Azure Cognitive Search</span>
          <span className="bg-green-100 text-green-800 px-2 py-1 rounded">🔍 Azure AI Search</span>
          <span className="bg-purple-100 text-purple-800 px-2 py-1 rounded">🏛️ Azure Front Door + WAF</span>
          <span className="bg-orange-100 text-orange-800 px-2 py-1 rounded">🔐 Azure AD B2C</span>
          <span className="bg-red-100 text-red-800 px-2 py-1 rounded">🛡️ Azure Content Safety</span>
          <span className="bg-gray-100 text-gray-800 px-2 py-1 rounded">💾 Cosmos DB Sessions</span>
        </div>
      </div>
      ...
      <div className="flex-shrink-0 border-t bg-white p-4">
        ...
        <div className="mt-2 text-xs text-gray-500 text-center">
          Powered by Azure Cognitive Search • Secured with Content Safety • Available 24/7
        </div>
      </div>
    </div>
  )
