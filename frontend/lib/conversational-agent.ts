...
export class ConversationalAgent {
  ...
  private async handleClaimFlow(
    ...
    if (this.isClaimIntent(userMessage) && !claimData.policyNumber) {
      ...
      const response =
        "🔵 I'll help you file your claim. " + CLAIM_STEPS[0].question;
  ...
  private async createClaim(sessionId: string): Promise<string> {
    ...
    // Store in governed claim workflow (simulated)
    await this.storeClaimInWorkflow(claimId, claimData);
...
You can check the status anytime by asking "What's the status of claim ${claimId}?"`;
  ...
  private async storeClaimInWorkflow(
    claimId: string,
    claimData: any,
  ): Promise<void> {
    // Simulate tenant-scoped claim workflow storage
    console.log(`📁 Storing claim ${claimId} in governed workflow:`, claimData);

    // In the greenfield implementation, this becomes:
    // 1. Store evidence in tenant-scoped private document storage
    // 2. Persist workflow state through the private AgentCore/backend service
    // 3. Audit the event for claims and compliance review
    // 4. Route to adjuster or underwriter queue
  }
