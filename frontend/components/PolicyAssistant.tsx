  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage: Message = {
        id: (Date.now() + 1).toString(),
        role: "assistant",
        content: `❌ Error: ${error.message}\n\n*Note: Policy Assistant requires the private SageSure policy workflow to be reachable for live answers.*`,
        timestamp: new Date(),
      };
    ...
    <div className="h-full flex flex-col bg-white">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-indigo-100">
                24/7 personalized policy guidance with AI
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
      ...
      <div className="flex-shrink-0 border-t bg-white p-4">
        ...
        <div className="mt-2 text-xs text-gray-500 text-center">
          Powered by SageSure policy knowledge • Governed controls • Available
          24/7
        </div>
      </div>
    </div>
